...
def make_prior_logit(onehots, smooth=1.0):
    """학습 구간 빈도 기반 물리적(?) 치우침을 prior 로짓으로 만듭니다."""
    counts = onehots.sum(axis=0) + smooth           # (45,)
    probs = counts / counts.sum()
    odds = probs / (1 - probs)
    return torch.tensor(np.log(odds), dtype=torch.float32)  # (45,)
...
# === NEW: accuracy / picking helpers ===
def set_accuracy_from_logits(logits, y_onehot, k=6):
    # logits, y_onehot: (B, 45) on same device
    # logits로 top-k 뽑아도 순위 동일(시그모이드는 단조증가)
    topk = torch.topk(logits, k=k, dim=1).indices
    pred_onehot = torch.zeros_like(logits)
    pred_onehot.scatter_(1, topk, 1.0)
    correct_hits = (pred_onehot * y_onehot).sum(dim=1)        # 각 샘플에서 맞춘 공 개수
    denom = y_onehot.sum(dim=1).clamp_min(1.0)                # 보통 6
    return (correct_hits / denom).mean().item()

def pick_numbers_from_logits(logits, k=6, gumbel=False):
    # logits: (45,)
    if gumbel:
        ...
        scores = logits
    topk = torch.topk(scores, k=k).indices + 1  # 1..45
    return torch.sort(topk).values.tolist()
...
class ResidualBlock(nn.Module):
    def __init__(self, width, drop_p=0.6):
        super().__init__()
        self.lin1 = nn.Linear(width, width*2, bias=False)
        self.lin2 = nn.Linear(width*2, width, bias=False)
        self.act = nn.GELU()
        self.norm = nn.LayerNorm(width)  # 과적합 목적이면 이거도 빼볼 수 있음
        self.drop = nn.Dropout(p=drop_p)  # 강한 드롭아웃로 고의적 불확실성
...
class BigDeepMLP(nn.Module):
    def __init__(self, input_size=45, width=1024, depth=12, drop_p=0.6, output_size=45):
        super().__init__()
        self.inp = nn.Linear(input_size, width)
        # 깊고 넓은 잔차 MLP
        self.blocks = nn.ModuleList([ResidualBlock(width, drop_p=drop_p) for _ in range(depth)])
        self.out = nn.Linear(width, output_size)
        # 출력에 시그모이드/소프트맥스 없음! (BCEWithLogitsLoss 쓸 거라 생로짓 유지)
...
def main():     
    ...
    for bn in ballset_list:
        ...
        prior_logit = make_prior_logit(numbers_onehot, smooth=2.0).to(device)
        ...
        model = BigDeepMLP(input_size=45 * win_size, width=1024, depth=12, drop_p=0.6, output_size=45).to(device)
        ...
        def init_weights(m):
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)  # kaiming보다 완만
        ...
        train_loader = data.DataLoader(
            train_dst, 
            batch_size = min(64, len(train_dst)), 
            shuffle = True,
            # num_workers=1,
            drop_last=False
        )
        ...
        for epoch in range(num_epochs):
            ...
            for (xs, ys) in train_loader:
                ...
                # === NEW: flatten (B, win_size*45)
                B, T, D = xs.shape                           # T=win_size, D=45
                xs = xs.view(B, T * D)

                logits = model(xs)                                   # (B, 45) - raw logits
                logits = logits + prior_logit                     # prior를 로짓에 더해 치우침 반영
                ...
                train_acc = set_accuracy_from_logits(logits, ys, k=6)
                epoch_train_correct += train_acc * B
                total_train_samples += B

            train_loss = epoch_train_loss / max(1, total_train_samples)
            train_accuracy = epoch_train_correct / max(1, total_train_samples)
            ...
            with torch.no_grad():
                ...
                # === NEW: all-tensor, flatten, no softmax
                x_valid_np = np.expand_dims(valid_onehots[-win_size-1:-1], 0)          # (1, win_size, 45)
                x_valid = torch.from_numpy(x_valid_np).to(device, dtype=torch.float32)
                y_correct = torch.from_numpy(np.array([valid_onehots[-1]])).to(device, dtype=torch.float32)
                # flatten
                Bv, Tv, Dv = x_valid.shape
                x_valid = x_valid.view(Bv, Tv * Dv)  

                val_pred = model(x_valid) + prior_logit
                # pred = F.softmax(pred, dim=1)
                
                val_loss = criterion(val_pred, y_correct)

                epoch_val_loss += val_loss.item()

                val_acc = set_accuracy_from_logits(val_pred, y_correct, k=6)
                epoch_val_correct += val_acc * x_valid.size(0)
                total_val_samples += x_valid.size(0)
            ...
            val_loss = epoch_val_loss / max(1, total_val_samples)
            val_accuracy = epoch_val_correct / max(1, total_val_samples)
            val_losses.append(val_loss)
            val_accuracies.append(val_accuracy)
        ...
        with torch.no_grad():
            last_window = torch.from_numpy(valid_onehots[-win_size:])  # (T, 45)
            x_test = last_window.unsqueeze(0).to(device, dtype=torch.float32)  # (1, T, 45)
            x_test = x_test.view(1, -1)  # (1, T*45)


            logits = model(x_test)[0] + prior_logit  # (45,)

        final_numbers = [pick_numbers_from_logits(logits, k=6, gumbel=True) for _ in range(5)]
        print("final_numbers : \n", final_numbers)
        print("")
