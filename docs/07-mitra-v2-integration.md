...
> **상태 (#5)**: 코드 구현 완료 — `model_m04_mitra/`, migration 005, 서버/프론트 모델 선택.
> 남은 일: 아이맥에서 Mitra-v2 가중치로 `m04_backtest.py --backend mitra` 실행 → §5.2 판정.
...
### 5.3 기준선 결과 (dev 100회차: 1101~1200, Mitra 제외)

| backend | logloss ↓ | brier ↓ | top15 적중 ↑ | 전략1 일치 ↑ |
|---|---|---|---|---|
| 균등 기대값 | 0.3927 | 0.1156 | 2.000 | 0.800 |
| uniform | 0.3927 | 0.1156 | 1.980 | 0.780 |
| m03 | 0.3973 | 0.1164 | 2.180 | 0.880 |
| logreg (컨텍스트 110회) | 0.3946 | 0.1160 | 1.970 | 0.770 |

- **m03 는 log loss 로 균등분포보다 유의하게 나쁘다** (차이 −0.0046, CI [−0.0084, −0.0010]).
  세트별 빈도의 우연한 흔들림을 편향으로 믿고 확률을 과신한 결과. 적중 수 차이(+0.2)는 CI 가 0 을 포함.
- logreg 로 입력 형태 6가지(컨텍스트 30/110/250회, same_set, 리허설 feature, 조합)를 돌려도 균등분포를 이긴 설정은 없다.
  전략1 일치 수가 설정에 따라 0.75~0.96 으로 흔들리는데, 이게 전부 CI 안의 잡음이다 → §5.2 의 다중 비교 경고가 실제로 필요한 이유.

### 5.4 입력 형태로 조절할 수 있는 것

Mitra 는 "주어진 표에서 패턴을 찾는" 모델이라, **표를 어떻게 만드느냐가 곧 성능**이다.
단, 입력 조절은 데이터에 있는 신호를 더 잘 찾게 할 뿐, 없는 신호를 만들지는 못한다.

| 손잡이 | 옵션 | 효과 |
|--------|------|------|
| 행 정의 | (회차, 번호) 이진분류 — 현재 | 45-class 는 `max_classes=10` 에 걸림. 회차 단위 wide 표(45열 one-hot)는 컨텍스트가 400행 남짓으로 줄어 ICL 에 불리 |
| 컬럼(가설) | `feature_names()` 편집, `--use-rehearsal` | 가장 큰 손잡이. 무관한 컬럼은 ICL 에서 잡음이 되므로 적을수록 좋다 (50개 한도) |
| 컨텍스트 범위 | `--context-rounds`, `--context same_set` | 최근성 vs 표본 수. 5,120행 넘으면 사전학습 범위 밖 |
| 값 표현 | 번호를 수치 vs 범주, 빈도를 비율 vs 순위 | Mitra 전처리가 수치 스케일에 민감할 수 있음 — 순위 변환이 흔한 개선 포인트 |
| 앙상블 | `--n-estimators`, `shuffle_features` | 열/행 순서를 바꿔 여러 번 예측 후 평균 → 분산 감소 |
| fine-tune | `--fine-tune` (모드 B) | 우리 데이터에 맞춰 가중치 조정. GPU 필요, 과적합 위험 가장 큼 |
| 출력 보정 | `normalize_round()` (합 6) | 회차당 정확히 6개라는 제약을 반영 — 이미 적용 |
...
> 실제 진행: 세 단계를 한 PR(#5)로 묶어 구현했다. m04 는 UI 에서 "실험" 라벨로만 노출되고 기본값은 m03 이며,
> m04 확률표가 없으면 자동으로 m03 로 폴백하므로 Mitra 판정 전에 머지해도 기존 동작은 바뀌지 않는다.
